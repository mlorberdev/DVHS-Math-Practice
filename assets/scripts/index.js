!(function mathfacts() {
  ...
  document.getElementById("set_skills").addEventListener("click", function () {
    ...
    switch (op) {
      ...
      case "add_int": ws = "➕ & ➖ Signed Numbers"; ins = "The rules for adding integers are simple:<br><br>positive ➕ positive 🟰 positive.<br><br>negative ➕ negative 🟰 negative.<br><br>Adding different signed integers (positive + negative OR negative + positive) is actually subtraction!  First, subtract the two numbers; then, give the answer the same sign as the bigger number.<br><br>To subtract integers, first, change the operation (the ➖) to addition (➕); next, change the sign on the second number to its opposite (if it's positive, make it negative -- and vice-versa); last, follow the rules for addition!<br><br>Be sure to include the negative sign if your answer is negative.<br><br>" + 
      "Las reglas para sumar números enteros son simples:<br><br>positivo ➕ positivo 🟰 positivo.<br><br>negativo ➕ negativo 🟰 negativo.<br><br>Sumar diferentes enteros con signo (positivo + negativo O negativo + positivo ) ¡es en realidad una resta!  Primero, resta los dos números; luego, dale a la respuesta el mismo signo que el número mayor.<br><br>Para restar números enteros, primero, cambia la operación (el ➖) a la suma (➕); a continuación, cambie el signo del segundo número por su opuesto (si es positivo, hágalo negativo y viceversa); ¡Por último, sigue las reglas para la suma!<br><br>Asegúrate de incluir el signo negativo si tu respuesta es negativa.<br><br>"; break;
